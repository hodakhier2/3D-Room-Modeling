...
const DraggableChair = ({ initialPosition, roomWidth, roomDepth, onDragStart, onDragEnd }) => {
  ...
  useFrame(() => {
    if (chairRef.current) {
      chairRef.current.position.set(position[0], position[1], position[2]);
      chairRef.current.rotation.set(rotation[0], rotation[1], rotation[2]);
    }
  });
